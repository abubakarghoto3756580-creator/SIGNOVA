...
const services = [
  ...
  { id: 3, title: 'Physiotherapy', description: 'Back pain, stroke recovery, muscle strengthening, post-op rehab', image: null },
          ...
          {services.map((service, index) => {
            return (
              <motion.div
                ...
                whileHover={{ y: -5 }}
                className="group relative bg-signova-white rounded-sm overflow-hidden flex flex-col transition-all duration-300 border border-transparent hover:border-signova-gold hover:shadow-[0_15px_30px_rgba(200,163,106,0.15)]"
              >
                <div className="relative w-full h-44 overflow-hidden">
                  {service.image ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-signova-gold/15 via-signova-ivory to-signova-gold/10 flex items-center justify-center">
                      <Activity size={48} strokeWidth={1} className="text-signova-gold/60" />
                    </div>
                  )}
                </div>
